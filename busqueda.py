...
class ColaNodosConPrioridad:
    ...
    def __contains__(self, nodo):
        return any(
            x[2].estado == nodo.estado and x[2].heuristica <= nodo.heuristica
            for x in self.nodos)
...
class BusquedaGeneral:
    ...
    def es_nuevo(self, nodo):
        return (nodo not in self.frontera and nodo not in self.explorados)
...
class BusquedaPrimeroElMejor(BusquedaGeneral):
    def __init__(self, f, detallado=False):
        ...
        self.explorados.__contains__ = types.MethodType(
            lambda self, nodo: any(x.estado == nodo.estado and x.heuristica <=
                                   nodo.heuristica
                                   for x in self), self.explorados)
